...
def map_label(label, classes, testing = False, opt = None):
    ...
    if testing == True:
        if opt.al_seed == 'original':
          ...
          matcontent = sio.loadmat(opt.dataroot + "/" + opt.dataset + "/" + opt.class_embedding + "_splits_" + opt.dataset + "_al_" + opt.al_seed + ".mat")
        # numpy array index starts from 0, matlab starts from 1
        all_names = matcontent['allclasses_names']
        all_labels = classes.numpy()
        orderedLabel_to_class = {}

        mapped_label = torch.LongTensor(label.size())
        for i in range(classes.size(0)):
            mapped_label[label==classes[i]] = i    
            orderedLabel_to_class[i] = all_names[all_labels[i]][0][0]

        print('\n\nTest classes ordered: ', orderedLabel_to_class)
        return mapped_label
...
class DATA_LOADER(object):
    ...
    def read_matdataset(self, opt):
        ...
        if not opt.validation:
            if opt.preprocessing:
                # only this branch is active always according to repo script files
