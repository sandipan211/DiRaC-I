...
if opt.gzsl:
    ...
    gzsl_folder = opt.work_dir + 'new_zsl_models/FREE/GZSL_al_lr' + str(opt.al_lr) + '/' 
    ...
    if opt.cq:
        fstr = report_folder + opt.fname + '_gzsl_' + opt.dataset  + '_' + opt.al_seed + '_cq' + str(opt.cq)
    else:
        fstr = report_folder + opt.fname + '_gzsl_' + opt.dataset  + '_' + opt.al_seed
...
#new change - made separate report folders
czsl_folder = opt.work_dir + 'new_zsl_models/FREE/CZSL_al_lr' + str(opt.al_lr) + '/' 
...
if opt.cq:
    fstr = report_folder + opt.fname + '_czsl_' + opt.dataset  + '_' + opt.al_seed + '_cq' +str(opt.cq)
else:
    fstr = report_folder + opt.fname + '_czsl_' + opt.dataset  + '_' + opt.al_seed 
...
for epoch in range(0,opt.nepoch):
    ...
    if opt.gzsl:   
        ...
        # gzsl_cls = classifier.CLASSIFIER(train_X, train_Y, data, nclass, opt.cuda, opt.classifier_lr, 0.5, 25, opt.syn_num, generalized=True, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096, testing = True, opt = opt)
        gzsl_cls = classifier_zero.CLASSIFIER(train_X, train_Y, data, nclass, opt.cuda, opt.classifier_lr, 0.5,25, opt.syn_num, netFR=netFR, dec_size=opt.attSize, dec_hidden_size=(opt.latensize*2), generalized=True, testing=True, opt=opt)

        # new change - best considered on the basis of H_new and not H
        if best_gzsl_H_new < gzsl_cls.H_new:
            best_gzsl_epoch= epoch
            best_acc_seen, best_acc_unseen, best_gzsl_H, best_gzsl_H_new, best_acc_common_unseen_gzsl, best_classwise_accs_gzsl, best_classwise_accs_common_unseen_gzsl = gzsl_cls.acc_seen, gzsl_cls.acc_unseen, gzsl_cls.H, gzsl_cls.H_new, gzsl_cls.acc_common_unseen_gzsl, gzsl_cls.classwise_accs_gzsl, gzsl_cls.classwise_accs_common_unseen_gzsl

        print('\n\nGZSL: seen=%.4f, unseen=%.4f, h=%.4f, common unseen=%.4f, considered h=%.4f' % (gzsl_cls.acc_seen, gzsl_cls.acc_unseen, gzsl_cls.H, gzsl_cls.acc_common_unseen_gzsl, gzsl_cls.H_new),end=" ")

        if epoch % 10 == 0:
            print('GZSL: epoch=%d, best_seen=%.4f, best_unseen=%.4f, best_h=%.4f, common unseen=%.4f, considered h=%.4f' % (best_gzsl_epoch, best_acc_seen, best_acc_unseen, best_gzsl_H, best_acc_common_unseen_gzsl, best_gzsl_H_new))
    ...
    #new change - added last 2 params
    zsl_cls = classifier_zero.CLASSIFIER(syn_feature, util.map_label(syn_label, data.unseenclasses), \
                    data, data.unseenclasses.size(0), opt.cuda, opt.classifier_lr, 0.5, 25, opt.syn_num, \
                    netFR=netFR, dec_size=opt.attSize, dec_hidden_size=(opt.latensize*2), generalized=False, testing = True, opt = opt)
    
    #new change
    acc, acc_common_unseen, classwise_accs, classwise_accs_common_unseen = zsl_cls.acc, zsl_cls.acc_common_unseen, zsl_cls.classwise_accs, zsl_cls.classwise_accs_common_unseen

    if best_acc_common_unseen < acc_common_unseen:
        best_zsl_acc = acc
    ...
    print('ZSL: unseen accuracy=%.4f, common unseen accuracy=%.4f' % (acc, acc_common_unseen))
...
#new change
if opt.gzsl:
    gzsl_test_res = copy.deepcopy(data.test_res)
...
data.test_res['total_acc'] = best_zsl_acc
...
if opt.gzsl:
    ...
    print('the best GZSL seen accuracy is', best_acc_seen)
    print('the best GZSL unseen accuracy is', best_acc_unseen)
    print('the best GZSL H is', best_gzsl_H)
    print('the best GZSL considered H is', best_gzsl_H_new)
    ...
    gzsl_test_res['total_HM'] = best_gzsl_H
    ...
    gzsl_test_res['considered_HM'] = best_gzsl_H_new
