...
class ALE():
	...
	def __init__(self, args):
		...
		# 	classes_list.append(all_names[i][0][0])
		...
		# until here, all labels were made in range(0, num_class in tr/val/test)
...
if __name__ == '__main__':
	...
	czsl_folder = '/home/gdata/sandipan/BTP2021/new_zsl_models/ALE/CZSL_al_lr' + str(args.al_lr) + '/' 
	...
	# while writing SUN_reports.txt, it ended abruptly so I could not see how the train-val-test classes are split and how the overlapping swaps were made (if any). Hence writing this snippet to check those
	
	# if args.dataset == 'AWA2':
	# 	ol_path = '/home/gdata/sandipan/datasets/Animals_with_Attributes2/overlapping_awa2.txt'
	# elif args.dataset == 'CUB':
	# 	ol_path = '/home/gdata/sandipan/datasets/CUB_200_2011/overlapping_cub.txt'
	# elif args.dataset == 'SUN':
	# 	ol_path = '/home/gdata/sandipan/datasets/SUN_2012/overlapping_sun.txt'

	# ol_data = pd.read_csv(ol_path, header = None)
	# overlapping_classes = ol_data[0].tolist()
	# print('\nInitial Overlapping: {} \nInitially Overlapping classes: {}\n\n'.format(len(overlapping_classes), overlapping_classes))
	# trains, vals, tests = clf.trainclass_names, clf.valclass_names, clf.testclass_names
	# print('\n\n\nTrain classes ({}): {}'.format(len(trains), trains))
	# print('\n\n\nVal classes ({}): {}'.format(len(vals), vals))
	# print('\n\n\nTest classes ({}): {}'.format(len(tests), tests))
	# still_overlapping = list(set(overlapping_classes).intersection(set(tests)))
	# print('\n\n\nFinal ImageNet overlapping ({}): {}'.format(len(still_overlapping), still_overlapping))
