...
# unlike others, this code always run CZSL, even if --gzsl is specified as a parameter
...
if opt.gzsl:
    ...
    gzsl_folder = '/home/gdata/sandipan/BTP2021/new_zsl_models/TFVAEGAN/tfvaegan-master/GZSL_al_lr' + str(opt.al_lr) + '/' 
    ...
    if opt.cq:
        fstr = report_folder + fname + '_gzsl_' + opt.dataset  + '_' + opt.al_seed + '_cq' + str(opt.cq)
    else:
        fstr = report_folder + fname + '_gzsl_' + opt.dataset  + '_' + opt.al_seed
...
if opt.cq:
    fstr = report_folder + fname + '_czsl_' + opt.dataset  + '_' + opt.al_seed + '_cq' +str(opt.cq)
else:
    fstr = report_folder + fname + '_czsl_' + opt.dataset  + '_' + opt.al_seed 
...
for epoch in range(0,opt.nepoch):
    ...
    print('[%d/%d]  Loss_D: %.4f Loss_G: %.4f, Wasserstein_dist:%.4f, vae_loss_seen:%.4f'% (epoch, opt.nepoch, D_cost.data.item(), G_cost.data.item(), Wasserstein_D.data.item(),vae_loss_seen.data.item()),end=" ")
    netG.eval()
    netDec.eval()
    netF.eval()
    ...
    if opt.gzsl:   
        ...
        gzsl_cls = classifier.CLASSIFIER(train_X, train_Y, data, nclass, opt.cuda, opt.classifier_lr, 0.5, \
                25, opt.syn_num, generalized=True, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096, testing = True, opt = opt)
    ...
    zsl_cls = classifier.CLASSIFIER(syn_feature, util.map_label(syn_label, data.unseenclasses), \
                    ...
                    generalized=False, netDec=netDec, dec_size=opt.attSize, dec_hidden_size=4096, testing = True, opt = opt)
